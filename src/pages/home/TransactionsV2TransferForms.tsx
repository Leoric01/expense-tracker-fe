...
import { FC, type SubmitEvent, useEffect, useRef, useState } from 'react';
...
export type FormMode = 'wallet-transfer' | 'asset-exchange';

export type TransactionsV2TransferFormsProps = {
  trackerId: string;
  /** `page` = odsazení jako na /transactions/prevody; `dialog` = bez vnějšího mt (modal má vlastní padding) */
  variant?: 'page' | 'dialog';
  /** Po načtení holdingů jednorázově předvyplní zdroj/cíl a záložku (např. z drag&drop na přehledu) */
  prefillSourceHoldingId?: string | null;
  prefillTargetHoldingId?: string | null;
  /** Pokud není zadáno, odvodí se z `assetCode` zdroje a cíle */
  prefillMode?: FormMode | null;
  /** Zvyš při každém novém páru, aby se předvyplnění zopakovalo i pro stejné ID */
  prefillSessionKey?: string | number;
  /** Po úspěšném vytvoření převodu nebo výměny (např. zavřít modal a invalidovat dashboard) */
  onSubmitted?: () => void;
};
...
export const TransactionsV2TransferForms: FC<TransactionsV2TransferFormsProps> = ({
  trackerId,
  variant = 'page',
  prefillSourceHoldingId = null,
  prefillTargetHoldingId = null,
  prefillMode = null,
  prefillSessionKey = 0,
  onSubmitted,
}) => {
  ...
  const prefillAppliedSigRef = useRef('');
  ...
  const holdingsContent = holdingsRes?.content;
  const holdings: HoldingResponseDto[] = holdingsContent ?? [];

  useEffect(() => {
    const src = prefillSourceHoldingId?.trim() ?? '';
    const tgt = prefillTargetHoldingId?.trim() ?? '';
    if (!src || !tgt || src === tgt) {
      prefillAppliedSigRef.current = '';
      return;
    }
    if (!holdingsRes) return;
    const list = holdingsContent ?? [];
    const hasSrc = list.some((h) => h.id === src);
    const hasTgt = list.some((h) => h.id === tgt);
    if (!hasSrc || !hasTgt) return;

    const sh = list.find((h) => h.id === src);
    const th = list.find((h) => h.id === tgt);
    const inferredMode: FormMode = (() => {
      if (prefillMode) return prefillMode;
      const a = sh?.assetCode?.trim().toUpperCase() ?? '';
      const b = th?.assetCode?.trim().toUpperCase() ?? '';
      if (!a || !b) return 'wallet-transfer';
      return a === b ? 'wallet-transfer' : 'asset-exchange';
    })();

    const sig = `${prefillSessionKey}|${src}|${tgt}|${inferredMode}`;
    if (prefillAppliedSigRef.current === sig) return;
    prefillAppliedSigRef.current = sig;
    ...
    prefillMode,
    prefillSessionKey,
    prefillSourceHoldingId,
    prefillTargetHoldingId,
  ]);
  ...
  const walletTransferMutation = useCreateWalletTransfer({
    mutation: {
      onSuccess: (res) => {
        ...
        void queryClient.invalidateQueries({ queryKey: ['holdingFindAll', trackerId], exact: false });
  ...
  return (
    <Box sx={variant === 'page' ? { mt: 2 } : undefined}>
      <Tabs
        value={mode}
